...
public class ViewUtils {
    ...
    public static <T> T findViewByClass(@NonNull final View rootView, @NonNull final Class<T> clazz) {
        ...
        } else if (ViewGroup.class.isAssignableFrom(viewClass)) {
            final ViewGroup targetView = (ViewGroup) rootView;
            for (int childIndex = 0; childIndex < targetView.getChildCount(); childIndex++) {
                final View childView = targetView.getChildAt(childIndex);
                final T lookedView = findViewByClass(childView, clazz);
                if (lookedView != null) {
                    return lookedView;
                }
            }
